...
- デザイナーとエンジニア間で「ボタンを押したら何が起きるか」を構造化テキストで共有できる
- Figmaでは表現しにくい条件分岐（disabled条件、エラー時の分岐など）が明確になる
- AIへの指示が「モーダルを開いて」から「set editModalOpen to true」に変わり、解釈の揺れがなくなる

### 今後の展望

- より複雑なアプリケーション（リアルタイム更新、認証フローなど）での検証
- デザイナーとエンジニアが共同編集するワークフローの実践
- SBPの仕様をより厳密に定義し、画一的な書き方ができるようにする（書きやすく読みやすくする）

### 実際の開発現場で想定される課題

- 既存プロダクトへの途中導入が難しい（新規開発向き）
- コーポレートデザインやデザインシステムとの統合に工夫が必要
- 細かいデザインのカスタマイズ性がない（色、余白、アニメーションなどは別途指定が必要）
- SBP自体の学習コスト
