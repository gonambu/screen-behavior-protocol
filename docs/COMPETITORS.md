...
「Figmaでも振る舞いを定義できるのでは？」という疑問に対する整理。

### Figmaでできること（プロトタイピング機能）
...
### Figmaで表現しにくいこと（ロジック的振る舞い）

- 条件分岐: `if (user.isAdmin) show AdminPanel`
- フォームバリデーションのルール
- API呼び出し → 成功/失敗 → トースト表示 → 遷移 のようなフロー
- ローディング/エラー/成功の状態管理
- データに基づく動的なリスト表示
...
| 観点 | Figma | SBP |
|------|-------|-----|
| **フォーマット** | 独自バイナリ/複雑なJSON | YAML（テキスト） |
| **AIが読める** | △ プラグイン経由で限定的 | ◎ そのまま渡せる |
| **AIが書ける** | × | ◎ |
| **Git管理** | △ ブランチ機能はあるが差分が見にくい | ◎ テキストベースで差分明確 |
| **PRレビュー** | × | ◎ |

### SBPとFigmaの関係

SBPはFigmaの**代替ではなく補完**である。

```
デザイナーの成果物:
  ├── Figma: 見た目（ビジュアル）+ 視覚的な遷移
  └── SBP:  ロジック的振る舞い（状態・条件・バリデーション・API連携）

両方を組み合わせることで、AIが正確にコードを生成できる
```
...
| **フレームワーク変換** | AIが抽象コンポーネントから推論 | 実装は利用者任せ |
